...
export function AddJobTaskButton({ jobId, jobTitle }: { jobId: string; jobTitle: string }) {
  return (
    <button
      type="button"
      onClick={() => openTaskForm({ entityType: 'job', entityId: jobId, entityLabel: jobTitle })}
...
export function JobNoteForm({ jobId }: { jobId: string }) {
  ...
  const submit = () => {
    ...
    startTransition(async () => {
      const res = await addNote({ entityType: 'job', entityId: jobId, body: body.trim() });
