    ...
    <main className="flex min-h-dvh items-center justify-center bg-surface px-4">
      <div className="w-full max-w-sm rounded-2xl border border-edge bg-panel p-8">
        <div className="mb-6 text-center">
          <div className="text-lg font-semibold text-ink">{businessConfig.displayName}</div>
          <div className="text-sm text-ink-3">Operations Dashboard</div>
        </div>

        {!configured ? (
          <div className="rounded-lg bg-amber-400/10 p-4 text-sm text-amber-300">
          ...
          <div className="rounded-lg bg-emerald-400/10 p-4 text-sm text-emerald-300">
            Check your inbox — we sent a sign-in link to <strong>{email}</strong>. It expires shortly.
          </div>
        ) : (
          <form onSubmit={onSubmit} className="space-y-4">
            <label className="block">
              <span className="mb-1 block text-sm font-medium text-ink-2">Work email</span>
              <input
                type="email"
                required
                autoComplete="email"
                ...
                placeholder="you@resinprosflooringllc.com"
            ...
            <button
              ...
              className="w-full rounded-lg bg-accent px-4 py-2 text-sm font-medium text-surface transition hover:bg-accent-soft disabled:opacity-50"
            >
              {status === 'sending' ? 'Sending link…' : 'Email me a sign-in link'}
            </button>
            {status === 'error' && <p className="text-sm text-bad">{message}</p>}
          </form>
        )}
        <p className="mt-6 text-center text-xs text-ink-4">Internal tool · authorized users only</p>
      </div>
    </main>
  );
}
